...
module "kms" {
  ...
  keys = [
    {
      key_ring_name     = local.enterprisedb_key_ring_name
      existing_key_ring = false
      keys = [
        {
          key_name                 = local.enterprisedb_key_name
          standard_key             = false
...
#   - only created if user passes a value for 'ibmcloud_kms_api_key' (used when KMS is in different account to EnterpriseDB)
#   - if no value passed for 'ibmcloud_kms_api_key', the auth policy is created by the EnterpriseDB module
#######################################################################################################################
...
locals {
  ...
  create_cross_account_kms_auth_policy        = !var.skip_enterprisedb_kms_auth_policy && var.ibmcloud_kms_api_key != null && !var.use_ibm_owned_encryption_key
  create_cross_account_backup_kms_auth_policy = !var.skip_enterprisedb_kms_auth_policy && var.ibmcloud_kms_api_key != null && !var.use_ibm_owned_encryption_key && var.existing_backup_kms_key_crn != null
  ...
  kms_key_crn       = var.use_ibm_owned_encryption_key ? null : var.existing_kms_key_crn != null ? var.existing_kms_key_crn : module.kms[0].keys[format("%s.%s", local.enterprisedb_key_ring_name, local.enterprisedb_key_name)].crn
  kms_key_id        = var.use_ibm_owned_encryption_key ? null : var.existing_kms_key_crn != null ? module.kms_key_crn_parser[0].resource : module.kms[0].keys[format("%s.%s", local.enterprisedb_key_ring_name, local.enterprisedb_key_name)].key_id
...
resource "ibm_iam_authorization_policy" "kms_policy" {
  ...
  roles                    = ["Reader"]
  description              = "Allow all EnterpriseDB instances in the resource group ${module.resource_group.resource_group_id} in the account ${local.account_id} to read the ${local.kms_service} key ${local.kms_key_id} from the instance GUID ${local.kms_instance_guid}"
  resource_attributes {
    name     = "serviceName"
...
resource "ibm_iam_authorization_policy" "backup_kms_policy" {
  ...
  source_resource_group_id = module.resource_group.resource_group_id
  roles                    = ["Reader"]
  description              = "Allow all EnterpriseDB instances in the resource group ${module.resource_group.resource_group_id} in the account ${local.account_id} to read the ${local.backup_kms_service} key ${local.backup_kms_key_id} from the instance GUID ${local.backup_kms_instance_guid}"
  resource_attributes {
    name     = "serviceName"
    operator = "stringEquals"
...
locals {
  create_sm_auth_policy = var.skip_enterprisedb_sm_auth_policy || var.existing_secrets_manager_instance_crn == null ? 0 : 1
}

# create a service authorization between Secrets Manager and the target service (Databases for EnterpriseDB)
resource "ibm_iam_authorization_policy" "secrets_manager_key_manager" {
  count                       = local.create_sm_auth_policy
  source_service_name         = "secrets-manager"
  source_resource_instance_id = local.existing_secrets_manager_instance_guid
  target_service_name         = "databases-for-enterprisedb"
  target_resource_instance_id = module.enterprisedb.guid
  roles                       = ["Key Manager"]
  description                 = "Allow Secrets Manager with instance id ${local.existing_secrets_manager_instance_guid} to manage key for the databases-for-enterprisedb instance"
}

# workaround for https://github.com/IBM-Cloud/terraform-provider-ibm/issues/4478
resource "time_sleep" "wait_for_enterprisedb_authorization_policy" {
  count           = local.create_sm_auth_policy
    ...
    for service_credentials in var.service_credential_secrets : {
      ...
      secrets = [
        for secret in service_credentials.service_credentials : {
          ...
          service_credentials_source_service_crn  = module.enterprisedb.crn
...
module "secrets_manager_service_credentials" {
  ...
  depends_on                  = [time_sleep.wait_for_enterprisedb_authorization_policy]
