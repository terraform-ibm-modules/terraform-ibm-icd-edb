# Configuring complex inputs in Databases for EnterpriseDB

Several optional input variables in the IBM Cloud [Databases for EnterpriseDB deployable architecture](https://cloud.ibm.com/catalog#deployable_architecture) use complex object types. You specify these inputs when you configure deployable architecture.
...
When you add an IBM Database for EnterpriseDB deployable architecture from the IBM Cloud catalog to IBM Cloud Project , you can configure service credentials. In edit mode for the projects configuration, from the configure panel click the optional tab.
...
- `service_credentials_source_service_role`: (required): The role to give the service credential in the Databases for EnterpriseDB service. Acceptable values are `Writer`, `Reader`, `Manager`, and `None`
...
The Configuration variable tunes the EnterpriseDB database to suit different use case. For more information, see [Configuration](https://cloud.ibm.com/apidocs/cloud-databases-api/cloud-databases-api-v4#setdatabaseconfiguration-request)).
...
- `maxmemory`: Determines the amount of data that you can store in EnterpriseDB as a percentage of the deployments memory. (default: `80`).
- `maxmemory-policy`: Determines eviction behavior when `maxmemory` limit is reached.
- `appendonly`: Enables EnterpriseDB persistence when set to `yes`, If you are caching data, you want to set this value to `no`. (default: `yes`).
- `maxmemory-samples`: Tunes LRU eviction algorithm when EnterpriseDB is configured as a cache..
- `stop-writes-on-bgsave-error`: EnterpriseDB stops accepting writes if it detects an unsuccessful backup snapshot. For caching, you can set to `no`. (default: `yes`).
